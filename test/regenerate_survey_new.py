...
修改说明：
- 更新配置以测试新的 deep_survey_analyzer.py 功能
- 支持 comprehensive 模式：保留所有强关系连通分量
- 新增连通分量统计和关系类型分布展示
- 支持配置演化路径探索深度、最小连通分量大小等参数
"""
...
def regenerate_survey(
    ...
    config = {
        ...
        # 剪枝模式配置 - 新增功能
        'pruning_mode': 'comprehensive',  # 'comprehensive' 或 'seed_centric'
        # comprehensive: 保留所有强关系连通分量（推荐）
        # seed_centric: 只保留与Seed连通的论文（原模式）

        # 强逻辑关系定义 - 基于Socket Matching
        'strong_relations': [
            'Overcomes',   # 攻克/优化（纵向深化）
            'Realizes',    # 实现愿景（科研传承）
            'Extends',     # 方法扩展（微创新）
            'Alternative', # 另辟蹊径（颠覆创新）
            'Adapts_to'    # 技术迁移（横向扩散）
        ],

        # 弱关系定义 - 需要剪枝
        'weak_relations': ['Baselines'],

        # 连通分量过滤
        'min_component_size': 3,  # 最小连通分量大小（过滤噪音簇）

        # 演化路径配置
        'min_chain_length': 3,  # 最小链条长度
        'exploration_depth': 5,  # 分化/汇聚模式的探索深度
        'max_threads': 5,  # 最大演化故事数量

        # 路径去重配置
        'path_overlap_threshold': 0.8  # 路径重叠阈值（动态调整）
    }

    analyzer = DeepSurveyAnalyzer(config=config)
    logger.info("  ✅ 分析器初始化完成")
    logger.info(f"     - 剪枝模式: {config['pruning_mode']}")
    logger.info(f"     - 最小连通分量大小: {config['min_component_size']}")
    logger.info(f"     - 演化路径探索深度: {config['exploration_depth']}")
    ...
    try:
        ...
        if 'pruning_stats' in result:
            ...
            # 显示连通分量统计（comprehensive模式特有）
            if stats.get('pruning_mode') == 'comprehensive':
                logger.info(f"\n  📈 强关系连通分量分析:")
                logger.info(f"     - 连通分量总数: {stats.get('strong_components_count', 0)}")
                logger.info(f"     - 包含种子的分量: {stats.get('components_with_seed', 0)}")
                logger.info(f"     - 最大分量大小: {stats.get('largest_component_size', 0)}")

            # 显示关系类型分布
            if 'relation_type_distribution' in stats:
                logger.info(f"\n  📊 关系类型分布:")
                rel_dist = stats['relation_type_distribution']
